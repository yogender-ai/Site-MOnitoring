...
const createMonitorLimiter = rateLimit({
  ...
  message: {
    error:
      "Too many monitors created from this IP, please try again after 15 minutes",
  },
    ...
    try {
      ...
      if (monitorCount >= 15) {
        return res.status(403).json({
          error:
            "Monitor limit reached. You can only create up to 15 monitors.",
        });
      }
      ...
      if (userRes.rows.length > 0) {
        ...
        if (notifyEmail) {
          await sendMonitorActionAlert(notifyEmail, "added", url).catch((err) =>
            console.error(err),
          );
        }
      }
...
app.delete("/api/monitors/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM monitors WHERE id = $1 AND user_id = $2", [
      id,
      req.user.id,
    ]);
    ...
    if (userRes.rows.length > 0) {
      ...
      const notifyEmail = u.notification_email || u.email;
      if (notifyEmail) {
        await sendMonitorActionAlert(notifyEmail, "deleted", url).catch((err) =>
          console.error(err),
        );
      }
    }
...
app.get("/api/monitors/:id/logs", requireAuth, async (req, res) => {
  ...
  try {
    const own = await pool.query(
      "SELECT 1 FROM monitors WHERE id = $1 AND user_id = $2",
      [id, req.user.id],
    );
...
app.get("/api/monitors/:id/logs/csv", requireAuth, async (req, res) => {
  ...
  try {
    const own = await pool.query(
      "SELECT name FROM monitors WHERE id = $1 AND user_id = $2",
      [id, req.user.id],
    );
    if (own.rows.length === 0) {
      return res.status(404).json({ error: "Monitor not found" });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="monitor-${id}-logs.csv"`,
    );
...
if (RENDER_URL) {
  setInterval(
    () => {
      ...
      const httpModule = RENDER_URL.startsWith("https")
        ? require("https")
        : require("http");
      httpModule
        .get(`${RENDER_URL}/api/keep-alive`, (resp) => {
